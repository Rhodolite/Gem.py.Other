...
#
#   Tree: Name, Version 1 - A name (in a specific context), Version 1
#
...
class Tree_Name_V1(object):
    ...
    def __repr__(self):
        return arrange('<Tree_Name_V1 @{}:{} {!r} {}>', self.line_number, self.column, self.id, self.context)


def create_Tree_Name_V1(line_number, column, id, context):
    assert fact_is_positive_integer   (line_number)
    assert fact_is_substantial_integer(column)

    assert fact_is_full_native_string(id)

    return Tree_Name_V1(line_number, column, id, context)
