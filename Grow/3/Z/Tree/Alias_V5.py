...
#
#           2)  `Tree_Symbol_Alias.as_name` is a `Parser_Symbol_0`.
#
#       Version 5:
#
#           1)  `Tree_Module_Alias.as_name` is a `Parser_Symbol`.
#
#           2)  `Tree_Symbol_Alias.as_name` is a `Parser_Symbol`.
#
#           3)  Instead of creating either a `Tree_Module_Alias` with a `.as_name` of `parser_none`; instead it simply
#               uses the `Parser_Module_Name` as the `Tree_Alias`
#
#           4)  Instead of creating either a `Tree_Symbol_Alias` with a `.as_name` of `parser_none`; instead it simply
#               uses the `Parser_Symbol` as the `Tree_Alias`
#
