...
#
@gem('Melanite.Development')
def gem():
    require_gem('Melanite.Core')
    require_gem('Melanite.Multiprocessing')


    #
    #   NOTE #1:
    #       See Note in 'Melanite.Child' on pickling 'pickable_child_calculate'
    #
    #   NOTE #2:
    #       In order to import 'Melanite.Child' we have to get around the Gem system, hence create a privileged function
    #       and do a normal python import :(
    #
    @privileged
    def import_pickable_child_calculate():
        from Melanite.Child import pickable_child_calculate
